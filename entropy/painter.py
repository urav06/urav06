...
from matplotlib.colors import LinearSegmentedColormap, is_color_like, to_rgb
...
NUM_STEPS   : int               = 80000
TRANSIENT   : int               = 5000      # RK4 burn-in steps discarded before render
DT          : float             = 0.005
CHUNK_SIZE  : int               = 500

# --- Lorenz Base Parameters ---
# RHO_BASE sits above the worst-case Hopf bifurcation (~26.3 for our σ/β jitter range);
# below that ρ the system collapses to a fixed point and the render is blank.
SIGMA_BASE      : float = 10.0
BETA_BASE       : float = 8.0 / 3.0
RHO_BASE        : float = 30.0
RHO_CHAOS_SCALE : float = 20.0

# --- Color Defaults ---
FALLBACK_COLOR  : str   = "#7B8B9F"     # used when Gemini returns an unparseable color
DARK_FACTOR     : float = 0.25          # cmap dark anchor as a fraction of mood color
DARK_FLOOR      : float = 0.08          # min RGB channel so the dark anchor stays visible
...
def paint(transmutation: Transmutation, output_path: Path) -> None:
    ...
    color       = transmutation.mood_color if is_color_like(transmutation.mood_color) else FALLBACK_COLOR

    _render(trajectory, color, view, output_path)
...
def _integrate(
    ...
    state       = initial.copy()
    total       = NUM_STEPS + TRANSIENT
    trajectory  = np.zeros((total, 3), dtype=np.float64)
...
def _make_cmap(hex_color: str) -> LinearSegmentedColormap:
    """ Build a gradient that stays visible on a black background even for dark moods. """
    rgb     = np.array(to_rgb(hex_color))
    dark    = tuple((rgb * DARK_FACTOR).clip(DARK_FLOOR, 1.0))
    return LinearSegmentedColormap.from_list("mood", [dark, hex_color, "white"])
